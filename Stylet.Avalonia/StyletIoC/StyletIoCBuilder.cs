using Stylet.Avalonia.StyletIoC.Internal;
...
public class StyletIoCBuilder : IStyletIoCBuilder
{
    ...
    public void Autobind(IEnumerable<Assembly> assemblies)
    {
        ...
        autobindAssemblies = existing.Concat(GetAssemblies(assemblies, "Autobind")).Distinct().ToList();
    }
    ...
    {
        module.AddToBuilder(this, GetAssemblies);
    }
    ...
    {
        var container = new Container(autobindAssemblies);

        // Just in case they want it
        var bindings = this.bindings.ToList();
        var containerBuilderBindTo = new BuilderBindTo(typeof(IContainer), GetAssemblies);
