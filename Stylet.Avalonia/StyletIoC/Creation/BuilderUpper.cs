...
public class BuilderUpper
{
    ...
    private Expression ExpressionForMember(Expression objExpression, MemberInfo member, Type memberType, ParameterExpression registrationContext)
    {
        ...
        var memberValue = parentContext.GetSingleRegistration(memberType, attribute.Key, true).GetInstanceExpression(registrationContext);
    ...
    public Action<IRegistrationContext, object> GetImplementor()
    {
        lock (implementorLock)
        {
            ...
            implementor = Expression.Lambda<Action<IRegistrationContext, object>>(GetExpression(typedParameterExpression, registrationContext, type), registrationContext, parameterExpression).Compile();
