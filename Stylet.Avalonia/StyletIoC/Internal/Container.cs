...
internal class Container : IContainer, IRegistrationContext
{
    ...
    {
        foreach (var value in registrations.Values)
        {
    ...
    public object Get(Type type, string key = null)
    {
        ...
        return Get(type.TypeHandle, key, type);
    }
    ...
    public IEnumerable<object> GetAll(Type type, string key = null)
    {
        if (type == null)
            throw new ArgumentNullException("type");
        return GetAll(type.TypeHandle, key, type);
    }
    ...
    public object GetTypeOrAll(Type type, string key = null)
    {
        if (type == null)
            throw new ArgumentNullException("type");
        return GetTypeOrAll(type.TypeHandle, key, type);
    }
    ...
    {
        var builderUpper = GetBuilderUpper(item.GetType());
    ...
    bool IRegistrationContext.CanResolve(Type type, string key)
    {
        ...
        if (this.registrations.TryGetValue(new TypeKey(type.TypeHandle, key), out registrations) ||
            ...
            TryCreateGenericTypesForUnboundGeneric(type, key, out registrations) ||
            TryCreateSelfBinding(type, key, out registrations))
        {
            return true;
        }
    ...
    private bool TryRetrieveGetAllRegistrationFromElementType(TypeKey elementTypeKey, Type collectionTypeOrNull, out IRegistration registration, Type elementTypeIfAvailable = null)
    {
        ...
        if (getAllRegistrations.TryGetValue(elementTypeKey, out registration))
            return true;
        ...
        registration = getAllRegistrations.GetOrAdd(elementTypeKey, x => new GetAllRegistration(listType.TypeHandle, this, elementTypeKey.Key));
        return true;
    }
    ...
    private bool TryCreateSelfBinding(Type type, string key, out IRegistrationCollection registrations)
    {
        ...
        if (!autobindAssemblies.Contains(type.Assembly))
            return false;
    ...
    private bool TryCreateFuncFactory(Type type, string key, out IRegistrationCollection registrations)
    {
        ...
        {
            var typeKey = new TypeKey(type.TypeHandle, key);
            foreach (var registration in GetRegistrations(new TypeKey(genericArguments[0].TypeHandle, key), true, genericArguments[0]).GetAll())
            {
    ...
    private bool TryCreateGenericTypesForUnboundGeneric(Type type, string key, out IRegistrationCollection registrations)
    {
        ...
        foreach (var unboundGeneric in unboundGenerics)
        {
            ...
            registrations = AddRegistration(new TypeKey(type.TypeHandle, key), registration);
        }
    ...
    internal IRegistrationCollection AddRegistration(TypeKey typeKey, IRegistration registration)
    {
        try
        {
            return registrations.AddOrUpdate(typeKey, x => new SingleRegistration(registration), (x, c) => c.AddRegistration(registration));
        }
    ...
    {
        if (abstractFactoryBuilder == null)
            abstractFactoryBuilder = new AbstractFactoryBuilder();

        return abstractFactoryBuilder.GetFactoryForType(serviceType);
    }

    public BuilderUpper GetBuilderUpper(Type type)
    {
        var typeHandle = type.TypeHandle;
        return builderUppers.GetOrAdd(typeHandle, x => new BuilderUpper(typeHandle, this));
    }
